  ...
  <header class="header-shoji sticky top-0 z-50">
    <div class="max-w-xl mx-auto px-5 py-4 flex items-center gap-4">
      ...
      <div class="flex-1 min-w-0">
        <h1 class="text-sm font-bold tracking-widest uppercase" style="color:#261e16; letter-spacing:0.18em;">
          Quiz Lexique Kendo
        </h1>
        <p class="text-xs tracking-wider mt-0.5" style="color:#6a5a48; letter-spacing:0.1em;">
          CAF · CNKDR · Japonais ↔ Français
        </p>
      </div>

      <!-- Score en quiz, bouton historique sinon -->
      <Transition name="score-slide" mode="out-in">
        <div v-if="store.screen === 'quiz'" key="score" class="text-right shrink-0">
        ...
        <button
          ...
          key="history"
          class="history-btn"
          :class="(store.screen === 'history' || store.screen === 'history-detail') ? 'history-btn--active' : ''"
...
.history-btn:hover {
  background: rgba(124, 51, 40, 0.10);
  border-color: rgba(124, 51, 40, 0.30);
  color: #7c3328;
}
.history-btn--active {
  background: rgba(124, 51, 40, 0.12);
  border-color: rgba(124, 51, 40, 0.40);
  color: #7c3328;
}

/* Transitions */
.score-slide-enter-active { transition: all 0.3s cubic-bezier(0.22, 0.61, 0.36, 1); }
.score-slide-leave-active { transition: all 0.18s ease; }
.score-slide-enter-from   { opacity: 0; transform: translateX(10px); }
.score-slide-leave-to     { opacity: 0; transform: translateX(10px); }
</style>
